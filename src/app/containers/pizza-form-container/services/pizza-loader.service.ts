...
export class PizzaLoaderService {
  ...
  loadPizzaForEdit(data: IPizzaFormInterface): void {
    ...
    for (const pizza of data.pizzas) {
      ...
      group.patchValue({
        size: pizza.size,
        toppings: this.prefillToppingsSelection(
          group.get('toppings').value,
          pizza.toppings as PizzaToppingsEnum[],
        ),
      });
    }
  }

  prefillToppingsSelection(
    toppings: IToppingItem[],
    selectedToppings: PizzaToppingsEnum[],
  ): IToppingItem[] {
